...
jpeg::DCTChannelOutput jpeg::FastCosineTransformer::applyTransform(ColourMappedBlock::ChannelBlock const& inputChannel) const{
    std::array<int8_t, BlockGrid::blockElements> offsetChannelData;
    for (size_t i = 0 ; i < BlockGrid::blockElements ; ++i){
        offsetChannelData[i] = inputChannel[i] - 128;
    }
...
jpeg::ColourMappedBlock::ChannelBlock jpeg::FastCosineTransformer::applyInverseTransform(DCTChannelOutput  const& inputChannel) const{
